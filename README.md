...
- **Test Isolation:** API-created users are cleaned up automatically after each test (register → run → delete).
...
```text
├── 1_tests/          # Test specs (Signup, Login, Account Info, Products UI/API, User API)
...
npx playwright test --project=chromium      # unauthenticated UI + API specs
npx playwright test --project=chromium-authenticated   # authenticated specs
...
Actively maintained. Currently expanding coverage to the **Cart & Checkout** flow (the authenticated project is already wired up in `playwright.config.ts` for this). CI/CD integration via GitHub Actions is planned next.
